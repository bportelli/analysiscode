...
% Constants
DELIM = '\t';
XLABELSUFFIX = {[],' Top is near',' Test is near'};
...
funcOpts = [{s1} {yVar} {splitVar}]; %Preparation for saving
...
for series = 1:length(fieldnames(Tables))
    ...
    % Prep the variables
    [StimLevels,NumPos,OutOfNum,ProportionCorrectObserved,StimLevelsFineGrain, PF] = varPrep(T312,thVar);
    % Evaluate Psychometric function (for 'normal' cases that's guess rate 50%, lapse rate 5%)
    [PsychFunOut, searchGrid, paramsFree, options] = getPsychFun();

    % Run bootstrap if requested and save results
    if BOOTS == 1
        [bootsOut, GoFOut] = bootsfun(ParOrNonPar);
        varsToStore = [varsToStore, 'bootsOut', 'GoFOut'];
    end
    
    %% Prepare the Output (for Saving, if desired)
        varsToStore = {'StimLevels','NumPos','OutOfNum','ProportionCorrectObserved',...
        'StimLevelsFineGrain','PF','PsychFunOut','searchGrid','paramsFree','options','funcOpts','values'};
    for vts = 1:length(varsToStore)
        varAndPFOut(series).(varsToStore{vts}) = eval(varsToStore{vts});
    end
...
disp('****************');
for m = 1:length(thresholds75)
    fprintf('Threshold %s: %0.2f \n',leg_labels{m},thresholds75(m));
end
disp('****************');

for ser = 1:length(leg_labels)
t25t50t75(1) = PF(varAndPFOut(ser).PsychFunOut.paramsValues,0.25,'Inverse'); % Calculate and display thresholds for 25%, 50% and 75% of the green curve
t25t50t75(2) = PF(varAndPFOut(ser).PsychFunOut.paramsValues,0.50,'Inverse');
t25t50t75(3) = PF(varAndPFOut(ser).PsychFunOut.paramsValues,0.75,'Inverse');
thl = [25 50 75];
for m = 1:length(t25t50t75)
    fprintf('Threshold from %s at %d: %0.2f \n',leg_labels{ser},thl(m),t25t50t75(m));
end
disp('****************');
end
...
%% Analysis Sub-functions
    ...
    function [StimLevels,NumPos,OutOfNum,ProportionCorrectObserved,StimLevelsFineGrain, PF] = varPrep(currentTable,thVar) %Prep the variables
        switch s1
    ...
    function [PsychFunOut, searchGrid, paramsFree, options] = getPsychFun()
        ...
        switch s1
            ...
            case 2 %Proportion Top Near
                ...
                searchGrid.beta = linspace(0,100,101);
                %searchGrid.beta = logspace(1,3,100);
                searchGrid.gamma = 0;  %may be vector or scalar
