...
function [] = analyse710_auto_combi(datacomb, expName_comb, expDateSess_comb, pn)

%sprintf('%0.0f',clock) %to give names

% notDemos = find(cellfun(@(x)isempty(x),(regexp(expName_comb,'demo')))); %find the indices of files that are not demo files
...
k=1;
        while k <= length(fieldnames(datacomb)) 
...
while k <= length(fieldnames(datacomb)) && ~isempty(regexp(expName_comb{k},'demo','ONCE'))
...
%% Sub-functions
    ...
    function [currentTable, cTix] = getCurrentTable()
        fnD = fieldnames(datacomb);
        disp([fnD, expName_comb'])
        disp('TABLE NAME');
        currentTable = fnD{k};
        disp(currentTable)
        cTix = ismember(fnD,currentTable);
        fprintf('Chosen Table: %s run on %s\n',expName_comb{cTix},expDateSess_comb{cTix});
        ...
        currentTable = datacomb.(currentTable);
    end
    ...
    function [fhand, tx] = makePlot(name)
        ...
        name = {name};
        plottitle = [AnaID ' ' name{1} ' ' expName_comb{cTix}];
