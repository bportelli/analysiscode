function [ T31, thresholds ] = SplitResponsesAndPlot(a)
...
mtip = sprintf('Split series based on...');
str = T31.Properties.VariableNames;
[s,~] = listdlg('PromptString',mtip,...
    'SelectionMode','single',...
    'ListString',str,'InitialValue',find(ismember(str,'Input')));
splitVar = str{s};
...
for series = 1:length(fieldnames(Tables))
    ...
    % Prep the variables
    [StimLevels,NumPos,OutOfNum,ProportionCorrectObserved,StimLevelsFineGrain] = varPrep(T312,thVar);
    ...
    [fhand, hands, tx] = makePlot(LineColour{series}, series, hands);
    ...
    legend(hands,'String',leg_labels); % Make a legend for the plot
...
%% Analysis Sub-functions
    ...
    function [fhand, phand, tx] = makePlot(LineCol, seriesNum, phand)
        
        fhand = figure(1);
        
        %if combi
        scatter(StimLevels,ProportionCorrectObserved,...
            'ko','MarkerFaceColor',LineCol,'SizeData',OutOfNum*5);
        %else
